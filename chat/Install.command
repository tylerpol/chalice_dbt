...
# everything real happens in install.sh.

cd "$(dirname "${BASH_SOURCE[0]}")" || exit 1

# Called through `bash` on purpose: that way only this file needs the executable
# bit, and unzip tools that drop it on install.sh cannot break the double-click.
bash ./install.sh "$@"
status=$?
...
if [ "$status" -ne 0 ]; then
  ...
  printf '  Finished. Double-click Start.command to run the app.\n'
fi
